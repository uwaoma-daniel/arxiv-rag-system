...
class PromptBuilder:
    ...
    def build_qa_prompt(
        ...
        """
        Build QA prompt.
        Mistral [INST] format for causal LMs.
        Plain instruction format for seq2seq (flan-t5).
        """
        history_str = ''
        ...
        if is_seq2seq:
            prompt = (
                f'Answer the following question about research papers. '
                f'Use only the information in the excerpts. '
                f'Include citations like (Author, Year).\n\n'
                f'{history_str}'
...
class LLMBackend:
    ...
    def generate(self, prompt: str, max_new_tokens: int = 800) -> str:
        ...
        if self._is_seq2seq:
            # flan-t5: text2text pipeline — keep tokens low for HF free tier
            result = self._pipeline(
                prompt,
                max_new_tokens=min(max_new_tokens, 200),
                do_sample=False,
                num_beams=1,               # greedy — less RAM than beam search
...
class RAGPipeline:
    ...
    def query(
        ...
        # Step 4: build prompt
        # Truncate context to 1500 chars on seq2seq to save RAM
        ctx = retrieval_result.context_string
        if getattr(self.llm, "_is_seq2seq", False):
            ctx = ctx[:1500]
        prompt = self.prompt_builder.build_qa_prompt(
            query=question,
            context=ctx,
            conversation_history=history,
            is_seq2seq=getattr(self.llm, "_is_seq2seq", False),
        )
