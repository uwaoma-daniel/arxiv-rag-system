...
class QdrantVectorStore(BaseVectorStore):
    ...
    def search(self, query_vector, top_k=10) -> List[SearchResult]:
        # qdrant-client >= 1.7.0 uses query_points(); older used search()
        try:
        ...
        return [
            SearchResult(
                ...
                text=r.payload.get('text', r.payload.get('chunk_text', '')),
                metadata={
                    k: v for k, v in r.payload.items()
                    if k not in ('chunk_id', 'text', 'chunk_text')
                },
                score=float(r.score),
            )
            for r in hits
    ...
    def get_count(self) -> int:
        ...
        # attribute name changed across qdrant-client versions
        count = (
